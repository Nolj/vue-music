...
// 手机号是否被注册
// export const phoneRegistered=({phone=''})=>get(`/cellphone/existence/check?phone=${phone}`)
// // 手机号登录
// export const phoneLogin=({phone='',password=''})=>get(`/login/cellphone?phone=${phone}&password=${password}`)
// // 发送验证码
// export const sendVerify=({phone=''})=>get(`/captcha/sent?phone=${phone}`)
// // 验证验证码
// export const verify=({phone='',captcha=''})=>get(`/captcha/verify?phone=${phone}&captcha=${captcha}`)
// // 登录状态
// export const loginState = ()=>get("/login/status")
// // 退出登录
// export const logout=()=>get("logout")

// // 注册修改密码
// export const register=({phone='',pwd='',cap='',name=''})=>(`/register/cellphone?phone=${phone}&password=${pwd}&captcha=${cap}&nickname=${name}`)
...
//用户注册接口
export const postSignUp =(model) => http.post('rest/users',model);

//用户登录接口
export const loginIn =(model) => http.post('user/login',model);

//获取数据
export  const getUserData =(id) => http.get(`rest/user/${id}`);

//修改用户信息
export  const changeUser =(model,id) => http.put(`rest/user/${id}`,model);
...
//单曲评论提交
export const serveSongComment = (model) => http.post(`/rests/songcomment`,model)
//单曲获取评论
export const  getServeSongComment = (tid) => http.get(`/rests/songcomment/${tid}`)


//mv评论提交
export const serveMvComment = (model) => http.post(`/rests/mvcomment`,model)
//mv获取评论
export const  getServeMvComment = (id) => http.get(`/rests/mvcomment/${id}`)

//专辑评论提交
export const serveAlbumComment = (model) => http.post(`/rests/albumcomment`,model)
//专辑获取评论
export const  getServeAlbumComment = (id) => http.get(`/rests/albumcomment/${id}`)



//------------------------------------------收藏相关接口-----------------------------------------
//单曲收藏
export const subServeLikedSong = (model) => http.post(`/rests/likedsong`,model)
//用户获取收藏的单曲
export const  getServeLikedSong = (user_id) => http.get(`/rests/likedsong/${user_id}`)
//单曲取消收藏
export const  deleteLikedSong = (tid) => http.delete(`/rests/likedsong/delete/${tid}`)

//歌单收藏
export const subServeLikedAlbum = (model) => http.post(`/rests/likedalbum`,model)
//获取收藏的歌单
export const  getServeLikedAlbum = (user_id) => http.get(`/rests/likedalbum/${user_id}`)
//歌单取消收藏
export const  deleteLikedAlbum = (tid) => http.delete(`/rests/likedalbum/delete/${tid}`)

//歌手收藏
export const subServeLikedArtist = (model) => http.post(`/rests/likedartist`,model)
//歌手收藏的歌单
export const  getServeLikedArtist = (user_id) => http.get(`/rests/likedartist/${user_id}`)
//歌手取消收藏
export const  deleteLikedArtist = (tid) => http.delete(`/rests/likedartist/delete/${tid}`)

//专辑收藏
export const subServeLikedCollection = (model) => http.post(`/rests/likedcollection`,model)
//专辑收藏的歌单
export const  getServeLikedCollection= (user_id) => http.get(`/rests/likedcollection/${user_id}`)
//专辑取消收藏
export const  deleteLikedCollection = (tid) => http.delete(`/rests/likedcollection/delete/${tid}`)


//评论删除
export const  songDeteleComment = (id) => http.delete(`rests/songcomment/${id}`)
export const  mvDeteleComment = (id) => http.delete(`rests/mvcomment/${id}`)
export const  albumDeteleComment = (id) => http.delete(`rests/mvcomment/${id}`)
