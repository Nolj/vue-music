        ...
        methods:{
          ...
          togglePlay(){
           ...
           formatSeconds(value){
            ...
            toPlay(id){
              if(id&&id !=this.id){
                  this.$store.commit('setId',this.AsidePlayList[this.listIndex].id);//播放列表里当前歌曲在歌单中的位置
                  this.getUrl(this.AsidePlayList[this.listIndex].id);
                  this.$store.commit('setPicUrl',this.AsidePlayList[this.listIndex].al.picUrl);//图片
                  this.$store.commit('setTitle',this.AsidePlayList[this.listIndex].name);//歌名
                  this.$store.commit('setArtist',this.AsidePlayList[this.listIndex].ar[0].name);//歌手
                  this.getLyric(this.AsidePlayList[this.listIndex].id);
                  if(this.loginIn){
                      getServeLikedSong(this.userId).then(res =>{
                          for(let item of res.data){
                              if(item.tid == id){
                                  this.$store.commit('setIsActive',true); //单曲用户是否收藏
                                  break;
                              }
                          }
                      })
            ...
            getUrl(id){
                songUrl(id).then(res =>{
                    this.$store.commit('setUrl',res.data[0].url);//歌曲地址
