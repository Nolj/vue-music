<template>
    <div class="wrapper">
      <div class="inp border-bottom">
        <el-input v-model="pwd" placeholder="请输入密码" ref="inputs" autofocus="autofocus" show-password/>
...
import {loginStatus,phoneLogin} from '../../../networks/index.js'
export default {
  name: 'phonePwd',
  data () {
    return {
  ...
  methods: {
     ...
     */
    _isSure () {
      phoneLogin({phone:this.phone, password:this.pwd})
        .then(res => {
          console.log(res)
          if (res.data.code === 200) {
            // 密码正确
